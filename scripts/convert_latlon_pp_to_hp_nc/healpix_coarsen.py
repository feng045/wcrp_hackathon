...
def map_regional_to_global(_da, src_zoom):
...
def map_global_to_regional(_da, src_ds_region, tgt_ds_store):
    logger.debug(_da.name)
    coords = {
        n: c for n, c in _da.coords.items()
        if n != 'cell'
...
def calc_tgt_weights(src_ds, src_ds_region, tgt_ds, tgt_zoom):
...
def coarsen_healpix_zarr_region(src_ds, tgt_store, tgt_zoom, dim, start_idx, end_idx, chunks, regional=False,
    ...
    if regional:
        src_ds_region = src_ds_region.map(map_regional_to_global, src_zoom=src_zoom)
    ...
    if regional:
        ...
        tgt_ds = tgt_ds.map(map_global_to_regional, src_ds_region=src_ds_region, tgt_ds_store=tgt_ds_store)
        if src_ds_region.time[0] == src_ds.time[0]:
            tgt_weights = calc_tgt_weights(src_ds, src_ds_region, tgt_ds, tgt_zoom)
...
if __name__ == '__main__':
    ...
    zarr_chunks = {'time': chunks2d[zoom - 1][0], 'cell': -1}
    ...
    coarsen_healpix_zarr_region(test_src_ds, test_tgt_store, zoom - 1, '2d', start_idx, end_idx, chunks2d,
                                regional=True)
